...
class DiceTest(unittest.TestCase, Dice):
    def test_roll(self):
        ...
        self.assertRegex(Dice.roller('3d99+66'),
                         r'^\(\d+ \+ \d+ \+ \d+\) \+ \d+ = [\d]+$')
        self.assertRegex(Dice.roller('1d99', 'Cool'), r'^投擲 Cool 的結果是 \d+$')
        self.assertRegex(Dice.roller('2d99', 'Cute'),
                         r'^投擲 Cute 的結果是 \d+ \+ \d+ = \d+$')
        self.assertRegex(Dice.roller('3d100', 'Passion'),
        ...
        self.assertEqual(Dice.roller('3dd6'), '格式必須是 NdM 或 NdM+K，例如 `!dice 2d6` 是投擲兩顆六面的骰子')
